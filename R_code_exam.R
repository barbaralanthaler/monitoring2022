...
solidago <- solidago %>% 
  ...
  filter(year == 2000 | year == 2018) %>%   # belonging to the years 2000 and 2018
...
# Control if the coordinates have the correct format
...
sdmdata <- data.frame(cbind(presence_background, rbind(presence, absvals)))

# Create the model
model <- glm(presence_background ~ temperature + moisture + rainfall, data = sdmdata)
